...
object IPApp {
  ...
  val IMAGE_CATEGORIES = List("cupcakes", "chocolates", "muffins", "icecreams", "soup", "pasta", "bread", "cookies", "omelet", "frenchfries")
  ...
  def classifyImage(sc: SparkContext, path: String): Double = {
    System.setProperty("hadoop.home.dir", "C:\\Users\\Pavankumar Manchala\\Downloads\\winutils.exe")
    val model = KMeansModel.load(sc, IPSettings.KMEANS_PATH)
    val vocabulary = ImageUtils.vectorsToMat(model.clusterCenters)
